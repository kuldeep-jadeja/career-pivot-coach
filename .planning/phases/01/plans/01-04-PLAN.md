...
files_modified: [
  ...
  "lib/scoring/layers/layer1-ai-exposure.ts",
  "lib/scoring/layers/layer2-task-automation.ts",
  "lib/scoring/layers/layer3-industry-speed.ts",
  "lib/scoring/layers/layer4-experience-level.ts",
...
must_haves:
  truths:
    - "Risk calculator returns scores 0-100 for any valid occupation"
    - "All 4 scoring layers contribute to final score with correct weights (35/35/15/15)"
    - "Job title fuzzy matcher returns 3-5 O*NET matches with confidence scores"
    - "Unit tests pass for 8+ diverse occupations"
    - "Risk bands (Low/Moderate/Elevated/High/Very High) correctly map to score ranges"
  artifacts:
    - "lib/scoring/config.ts with tunable weights"
    - "lib/scoring/types.ts with RiskScore, JobMatch interfaces"
    - "lib/scoring/layers/ with 4 layer implementations"
    - "lib/scoring/risk-calculator.ts main scoring function"
    - "lib/scoring/job-matcher.ts fuzzy matching function"
    - "Vitest test suite with 8 hardcoded test occupations"
  key_links:
    - "risk-calculator.ts imports all 4 layer functions"
    - "risk-calculator.ts uses weights from config.ts"
    - "job-matcher.ts uses O*NET data from lib/data/onet-loader.ts"
---
